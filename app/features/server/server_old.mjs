...
export async function startServer({
    ...
    function checkAlive(ws) {
        ...
        ws.heartbeatTimeout = setTimeout(async () => {
            if (ws.roomId && await roomExists(ws.roomId)) {
     ...
     */
    /**
     * Generates a unique random room id and claims the room in the database using a transaction.
     * @throws if the Firestore database is not initialized or if a unique room cannot be claimed
     */
    async function generateAndClaimRoomId(ws, lat, lng) {
        ...
        let roomId, success = false, attempts = 0;
        while (!success && attempts < 10) {
            roomId = Array.from({ length: 4 }, () => chars[Math.floor(Math.random() * chars.length)]).join('');
            const roomRef = collection(db, roomId);
            const markerRef = doc(roomRef, 'marker');
            await runTransaction(db, async (transaction) => {
                const markerDoc = await transaction.get(markerRef);
                if (!markerDoc.exists()) {
                    transaction.set(markerRef, { roomCreatedAt: serverTimestamp() });
                    success = true;
                    ws.roomId = roomId;
                    const userObject = { joinedAt: serverTimestamp(), lost: false };
                    const userRef = doc(roomRef);
                    ws.id = userRef.id;
                    transaction.set(userRef, userObject);
                    ws.roomSnapshot = onSnapshot(roomRef, snap => {
                        snap.docChanges().forEach(change => {
                            const type = change.type;
                            const userDoc = change.doc;
                            const userId = userDoc.id;
                            if (type === 'added' && userId !== ws.id) {
                                const unsubscribe = onSnapshot(collection(roomRef, userId, 'locations'), locSnap => {
                                ...
                                if (userId !== ws.id) {
                                    ...
                                    const index = ws.locationSnapshots.findIndex(({ userId: _userId }) => _userId === userId);
                                    if (index !== -1) {
                            ...
                            } else if (type === 'modified' && userId !== ws.id && userDoc.data().lost) ws.send(JSON.stringify({
                                type: 'lost',
                                userId
                            }));
                        });
                    });
                    transaction.set(collection(userRef, 'locations'), { lat, lng, time: serverTimestamp() });
                }
            });
            attempts++;
        }
        if (!success) throw new Error('Failed to generate a unique room ID after 10 attempts');
    }
    ...
    async function joinRoom(ws, lat, lng, time) {
        ...
        console.log('Has user id when it joins:', !!ws.id);
    ...
    async function leaveRoom(ws) {
        if (!ws) throw new WebSocketError();
        if (!db) throw new DatabaseNotInitializedError();
        console.log('Has user id when it leaves:', !!ws.id);
        wsHasIdAndRoomId(ws);
